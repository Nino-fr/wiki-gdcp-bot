    ...
    if (filter) {
      ...
      try {
        ...
        await axios.default
          .get(
            encodeURI(
              `https://gardiens-des-cites-perdues.fandom.com/fr/wiki/Spécial:Recherche?query=${toSearch.trim()}&scope=internal`
            )
          )
          .then(async (res) => {
            if (
        ...
        await axios.default
          ...
          .then(async (res) => {
            ...
            if (titles.length === 0) {
              ...
              embed.title = `Résultats de la recherche pour \`${toSearch.trim()}\` dans les articles`;
              await titles.splice(20);
              ...
              for (let i = 0; i < titles.length; i++) {
                if (i === titles.length - 1) {
                  embed.addField(
                    ...
                    `[Article](${
                      links[i]
                    })\n\n[Voir tous les résultats](https://gardiens-des-cites-perdues.fandom.com/fr/wiki/Spécial:Recherche?scope=internal&query=${toSearch.trim()})`
                  );
                } else {
                  embed.addField(titles[i], `[Blog](${links[i]})`);
                }
