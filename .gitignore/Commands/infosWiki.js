...
class Infos extends Command {
  ...
  async run(message) {
    const msg = await message.channel.send(
      '<a:chargement_bleu:756866817917714453> Récupération des informations...'
    );
    try {
      const embed = new MessageEmbed()
        ...
        .addField('Pages au total', await this.bot.wiki.getTotalPages(), true)
        .addField("Nombre d'utilisateurs", await this.bot.wiki.getUsers(), true)
        .addField(
          "Nombre d'articles",
          await this.bot.wiki.getArticlesCount(),
          true
        )
        .addField(
